...
コンピュータでは、目に見えない文字や特殊な意味を持つ文字を表現するために、エスケープ文字を用いる。  
LinuxやRでもよく使われる。
...
| \n                      | 改行                         | 1行下に移動                    |
| \t                      | タブ (tab)                   | 横方向に空白を入れる           |
| \                       | スペース                     | ファイル名に空白がある場合など |
| \\|バックスラッシュ自身 | \ を文字として表現したい場合 |
| \"                      | ダブルクォーテーション       | " " を文字として扱う           |
...
fileディレクトリ以下に`test`ディレクトリを作ってみてください。

```bash
dirA $ cd ~/work/NGS2026/data 
data $ ls
Ahal 		file 
data $ mkdir test
data $ ls
...
使い方: `mv [option] <file name> <new name>`  

```bash
dirA $ cd ../../test
test $ ls
file1.txt
test $ mv file1.txt file8.txt
test $ ls
file8.txt    #できた
test $
```
...
```bash
test $ cd ../file/dirA　#dirAに移動
dirA $ cat file1.txt     
                     #file1.txtの中身を表示

dirA $ cat file1.txt file2.txt　
　　　　　　　　　　　　　#file1.txtとfile2.txtを繋げて表示
dirA $
```
...
さっき結合して表示した内容を"merged.txt"として出力してみる。

```bash
dirA $ cat file1.txt file2.txt > merged.txt　　　
dirA $ cat merged.txt
...
```bash
dirA $ cat ../dirB/file3.txt >> merged.txt
dirA $ cat merged.txt
